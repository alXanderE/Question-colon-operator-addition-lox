...
//> Statements and State expression
    return assignment();
//< Statements and State expression
// comma operator change for question 1
    return comma();
  }
...
//> equality
  private Expr equality() {
    if (match(BANG_EQUAL, EQUAL_EQUAL)) {
        ...
        comparison(); // Parse and discard right-hand operand.
        return equality();
    }
...
//> comparison
  private Expr comparison() {
    if (match(TokenType.GREATER,
        ...
        Token operator = previous();
        error(operator, "Missing left-hand operand.");
        term(); // Parse and discard right-hand operand.
        return comparison();
    }
...
//> term
  private Expr term() {
    if (match(TokenType.MINUS, TokenType.PLUS)) {
        Token operator = previous();
        error(operator, "Missing left-hand operand.");
        factor(); // Parse and discard right-hand operand.
        return term();
    }
...
//> factor
 private Expr factor() {
    if (match(TokenType.SLASH, TokenType.STAR)) {
        Token operator = previous();
        error(operator, "Missing left-hand operand.");
        unary(); // Parse and discard right-hand operand.
        return factor();
    }
